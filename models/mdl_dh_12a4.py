...
class Net(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.pos_emb = create_sinusoidal_embeddings(1 + 2 * cfg.slicelocation_clip, 32, pow_ = 10000)

        self.backbone_out += self.level_emb.embedding_dim + self.pos_emb.embedding_dim
    ...
    def forward(self, batch):
        ...
        x_posemb = torch.zeros_like(x_hidden[:,:,:self.pos_emb.embedding_dim])
        x_posemb[:] = self.pos_emb(batch['slicelocation'])

        x_hidden = torch.cat((x_hidden, x_emb,  x_posemb), -1)
        ...
        #loss_r = self.loss_fn(logits_r_end,y[condition_idx==0,1].long())

        logits_t1 = torch.stack((logits_l,logits_r)).permute(1,0,2)
        logits_t2 = torch.stack((logits_t2, logits_t2)).permute(1,0,2)

        logits = torch.zeros_like(torch.cat((logits_t1, logits_t2)))
        logits[condition_idx==0] = logits_t1
        logits[condition_idx==1] = logits_t2
        ...
        outputs['loss'] = (loss_r + loss_l + loss_t2 ) / 3
