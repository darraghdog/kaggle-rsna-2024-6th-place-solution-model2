...
#         print("Group", cfg.group)
    ...
    for epoch in range(cfg.epochs):
        ...
        if cfg.train:
            ...
            for itr in progress_bar:
                ...
                if cfg.local_rank == 0:
                    if i <= cfg.viz_first_batches:
                        cfg.viz_batch(data, cfg, i)

                if hasattr(cfg, 'unfreeze_after_epoch'):
                    if  epoch > cfg.unfreeze_after_epoch:
                        print('unfreezing model weights')
                        for param in model.parameters():
                            param.requires_grad = True
                        del cfg.unfreeze_after_epoch
                ...
                if cfg.local_rank == 0 and cfg.curr_step % cfg.batch_size == 0:
                    ...
                    if cfg.neptune_run:
                        for l in loss_names:
                            cfg.neptune_run[f"train/{l}"].log(value=output_dict[l].item() * cfg.grad_accumulation, step=cfg.curr_step)
    ...
    if cfg.test:
        run_eval(model, test_dataloader, cfg, pre="test")
#         upload_s3(cfg)
...
if __name__ == "__main__":
    ...
    cfg.viz_batch = importlib.import_module(cfg.viz).viz_batch
