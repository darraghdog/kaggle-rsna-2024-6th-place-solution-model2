...
class CustomDataset(Dataset):
    def __init__(self, df, cfg, aug, mode="train"):
        ...
        if (getattr(cfg, 'full_test_set', False) & cfg.val):
            trncdftst = self.trncdf['study_id series_id'.split()].drop_duplicates()
            trncdfls = []
    ...
    def __getitem__(self, idx):
        ...
        keep_running = True
        while keep_running:
            xy_pairs = torch.from_numpy(label.clip(0,999999)).reshape(2, -1)
            if self.aug:
                try:
                    aug_out = self.aug(image=center_img, keypoints = xy_pairs.numpy())
                    axial_kps = np.array(aug_out['keypoints']).flatten()
                    axial_img = aug_out['image']
                except:
                    print('Hitting the ds_dh_11g except')
                    axial_img = cv2.resize(center_img, (self.cfg.img_size,self.cfg.img_size), interpolation=cv2.INTER_CUBIC)
                    axial_kps = xy_pairs.flatten().numpy() * self.cfg.img_size /  center_img.shape[0]
